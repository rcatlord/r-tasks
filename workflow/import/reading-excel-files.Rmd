...
<p>Next we'll list all of the worksheets in the spreadsheet using the `excel_sheets()` function.</p>
```{r}
excel_sheets(path)
```

<p>We are interested in the worksheet called "MYE 6" which contains the median age of local authority populations in the UK. So let's use the `read_excel()` function and specify "MYE 6" in the `sheet` argument to load the worksheet.</p>
```{r}
...
<p>We have read the Excel file locally but you may want to download it directly from a URL. To do this we can use the `GET()` function from the <a href="https://cran.r-project.org/web/packages/httr/index.html" target="_blank">httr</a> package.</p>
```{r, eval = FALSE}
library(httr)
